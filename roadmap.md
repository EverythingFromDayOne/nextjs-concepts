...
Articles are authored as `.md.tpl` with `{EXTRACT:}` tokens and delivered alongside their demo sources. The `.md` is a build artifact and is never edited by hand. **`.tpl`-native is the standard from article 5 onward** (and for 3, 4, 7, 8+). Article 2's conversion debt is cleared as of session 8: the payload measurement across the stage-2 boundary that blocked it landed alongside the rewrite, and it is `.tpl`-native like the rest. The `unsourced-other` count in `verify-code-blocks` should fall toward zero and never grow.
...
**A template stages flat in `prompts/`, not `docs/`, until its demos exist.** `verify:templates` (`build-article.py --check --all`) only scans `.md.tpl` files under `docs/`, so a template whose `{EXTRACT:}` tokens point at demo files a session hasn't written yet breaks the gate for everyone the moment it lands — that happened with article 8's template. The session that authors the template keeps it staged flat in `prompts/` alongside its own prompt, writes the demos, and **moves** the template into its final `docs/concepts/<folder>/` location as the session's last step, once every extract target resolves. An in-flight template never breaks the chain because the gate never sees it.
