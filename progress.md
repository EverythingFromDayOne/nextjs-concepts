...
- [ ] `description` became required frontmatter in session 17, but `docs/templates/ARTICLE_TEMPLATE.md`, `docs/templates/RECIPE_TEMPLATE.md`, and roadmap §5's frontmatter list all still omit it — the next article authored from either template ships without the field and fails the corpus-web build. Out of scope for a pass restricted to article files; needs its own change.
...
### 17 — 2026-08-16 — `description` frontmatter pass (cross-repo prompt)

- Ran `prompts/description-pass.md` here: one `description` key per article, inserted immediately after `article_id` / `recipe_id`, nothing else touched. 10 articles (9 concepts + recipe #2) but **20 files**, because every article is `.tpl`-native — the key goes in the `.md.tpl` and `build-article.py --all` rewrites the `.md`. Editing the artifact directly would land the key and drift `verify:templates` in the same change.
- Deks run 117–126 characters (prompt bar: 90–160), sentence case, no trailing period, each drawn from the article's own H1 plus lead callout so none asserts anything the article doesn't already claim. `tags-and-invalidation` is the only quoted scalar — its dek contains `: `, which plain YAML would read as a mapping.
- Checked all ten against the 24 Ban=`yes` ledger tokens **before** writing: `verify-legacy-markers.mjs` has no frontmatter awareness, so a dek naming a dead surface would hard-fail with no allowed context available to sit in. None needed one.
- **Needs author: none.** Every article carries frontmatter, an id key, and a body H1, so both of the prompt's skip categories (untitled, no id key) are empty here. Deliberately not touched: `docs/recipes/index.md` (index page — `title` + `status`, no `recipe_id`), `docs/evolution-ledger.md` (no frontmatter), and the three templates still staged flat in `prompts/` (`cache-lifetimes`, `use-cache-directive`, `everything-went-dynamic-after-the-upgrade`) — pre-`docs/` per the §5 staging convention, so `verify:templates` never sees them and corpus-web never renders them. They need the field at conversion time, not now.
- Verified as the prompt specifies: `git diff --stat` = 20 files × 1 insertion, `git diff | grep '^-'` empty, and every changed file re-parsed with `js-yaml` (no `gray-matter` in this tree) to confirm `description` parses, sits second, and no `status` / `related` value shifted.
- Gate chain green and unchanged from the pre-pass baseline: `verify:templates` (10 templates, 0 drift), `verify:code-blocks --strict` (112 sourced / 10 unsourced-shell / 6 unsourced-other, 0 failures), `verify:links` (0 hard failures), `verify:legacy` (0 failures, 2 legacy-ok regions), demo build (route annotations unchanged), demo test.
