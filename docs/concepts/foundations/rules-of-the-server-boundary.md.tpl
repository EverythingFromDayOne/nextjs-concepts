...
Every row below was produced by writing the violation, building, and — where it built — requesting the route. Nothing here is inferred from documentation. Probes are numbered 1–11; probe 3 splits into two rows (3a, 3b) below, because — unlike every other row here — its build-vs-runtime outcome turned out not to be a fixed property of the violation.
...
| 3a | `cookies()` inside a cached scope, called from a route with no outer dynamic API of its own | **fails** — `Route … used cookies() inside "use cache". Accessing Dynamic data sources inside a cache scope is not supported.`, with a source frame naming the exact line | n/a | no |
| 3b | the identical violation, called from a route already dynamic for an unrelated reason | passes — the unrelated dynamic API already skips the build-time prerender before it reaches the cached call | **fails** — same error, carried as a `digest` inside the streamed RSC payload; the HTTP status is 200 | no — loud, but only once the route actually runs |
...
**Probe 3's enforcement phase is non-local — it depends on the calling route, not on the cached function.** 3a and 3b are the exact same violation (`cookies()` read inside `getDashboardLeakA()`'s `'use cache'` scope) reached two different ways. In 3a, the calling route (a scratch page that does nothing but await the cached function) has no dynamic API of its own, so Next attempts a full build-time prerender, walks into the cached function, and the violation is a hard build failure with a source frame pointing at the exact `cookies()` call. In 3b — the original, single-row version of this probe — the calling route (`app/leak-a`) *also* reads `cookies()` itself, outside the cache scope, which already makes that route dynamic; the build never attempts to prerender past that point, so the violation inside the cached function is never evaluated until a real request resumes the deferred render, where it fails the identical way but only then. The cached function cannot see or control which of these is true — it has no idea whether something upstream already opted its caller out of prerendering. Whether this specific rule is a build guarantee or a production landmine is decided by code outside the function that breaks it.
...
> **Verification status.** Verified against `next@16.3.0`. The enforcement matrix is measured in full — every row from a build, and a request where the build succeeded. Probe 8 and probe 9 were each re-run with an extra variant beyond the original eleven, because the first pass surfaced a generic error unrelated to the thing being tested; both are reported in full under the matrix. Probe 3 was re-run for a different reason than 8 and 9: its original single-row result (build passes, runtime fails) turned out to describe the calling route rather than the violation itself, so it's now 3a/3b — the identical violation reached from a route with no dynamic API of its own, which surfaces as a build failure instead. The bounded-staleness unification of the two sync-IO results is marked in-text as **my inference** from measured behavior rather than documented rationale. **Probe 10 was the load-bearing row, and it resolved:** a class instance crossing the client boundary is a hard build failure, not silent. `foundations/server-and-client-components` overclaimed a silent degradation and has been corrected in the same change that landed this article. Every code block here is extracted; the matrix was authored from measurements by the session that ran them, not by the article's author.
